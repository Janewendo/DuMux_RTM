...
int main(int argc, char** argv) try
{
    ...
    vtkWriter.addField(problem->getCalcium(), "CCalcium");
    // vtkWriter.addField(problem->getUrea(), "CUrea");
            ...
            if (row == "EndTimes")
            {
                ...
                while (row != "#")
                {
                    if (row != "#")
                    {
                        ...
                        ist >> tempTime;
                        // injectionParameters_ contains the time in seconds, MICP columns in hours!
...
//     auto linearSolver = std::make_shared<LinearSolver>(leafGridView, gridGeometry->dofMapper());
	...
	// using LinearSolver = Dumux::UMFPackBackend; //SuperLUBackend;
    // auto linearSolver = std::make_shared<LinearSolver>();
    ...
    {
		double minnewdt_;		
		minnewdt_ = problem->getnewdt(x); // getnewdt(*gridVariables) 
		printf("The value of timeLoop->timeStepSize is: %.12e\n", timeLoop->timeStepSize()); 
		
		// set time for problem for implicit Euler scheme
		// if (minnewdt_ > timeLoop->timeStepSize())
		// {
		...
		// else
		// {
        // problem->setTime( timeLoop->time() + minnewdt_ );
        // problem->setTimeStepSize( minnewdt_ );
		// timeLoop->setTimeStepSize(minnewdt_);
        // }
		...
		// printf("The value of timeLoop->timeStepSize2 is: %.12e\n", timeLoop->timeStepSize()); 
	    printf("The value of minnewdt1_ is: %.12e\n", minnewdt_); 
        ...
        // printf("The value of advanceTimeStep is: %.12e\n", advanceTimeStep()); 
		
        // advance to the time loop to the next step
        timeLoop->advanceTimeStep();

        //only write output when desired
        if (hasParam("Problem.Inverse") && getParam<bool>("Problem.Inverse") == true)
        {
            //measurements are at the end of episodes -9x, only output when we have measurements!
            if (timeLoop->isCheckPoint())
                if (problem->injectionType(episodeIdx_)<-89
                    && problem->injectionType(episodeIdx_)>-100)
                {
                    // update the output fields before write
                    problem->updateVtkOutput(x);
        ...
        {
            // changed by Du
			// if (timeLoop->timeStepIndex() % 10 == 0
                ...
                )
            {
                ...
                vtkWriter.write(timeLoop->time());
				printf("time.\n");
			    printf("The value of time is: %.8lf\n", timeLoop->time());
            }
        }
		...
		// if ( nonLinearSolver.suggestTimeStepSize(timeLoop->timeStepSize()) < minnewdt_ )
        // {
		timeLoop->setTimeStepSize(nonLinearSolver.suggestTimeStepSize(timeLoop->timeStepSize()));
		// }
	    // else
		// {timeLoop->setTimeStepSize(nonLinearSolver.suggestTimeStepSize(minnewdt_));}
	    
		// printf("The value of minnewdt2_ is: %.12e\n", minnewdt_); 
		// printf("The value of suggestdt is: %.12e\n", nonLinearSolver.suggestTimeStepSize(timeLoop->timeStepSize())); 
		// }
