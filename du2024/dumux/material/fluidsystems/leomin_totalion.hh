...
#include <dumux/material/components/iron3ion.hh>
...
#include <dumux/material/binarycoefficients/h2o_co2.hh> //added by du
#include <dumux/material/binarycoefficients/n2_co2.hh> //added by du
...
class LeoMinFluid
: public Base<Scalar, LeoMinFluid<Scalar, CO2Table, H2OType, Policy> >

{
    using ThisType = LeoMinFluid<Scalar, CO2Table, H2OType, Policy>;
...
public:
	...
	// using CO2 = Components::CO2<Scalar>;
    ...
    using Fe3 = Components::Iron3Ion<Scalar>;
    // using Brine_CO2 = BinaryCoeff::Brine_CO2<Scalar, CO2Table, true>;
    // using H2O_CO2 = BinaryCoeff::H2O_CO2<Scalar, CO2Table>;
	// using N2_CO2 = BinaryCoeff::N2_CO2<Scalar, CO2Table>;
	using H2O_CO2 = BinaryCoeff::H2O_CO2;
	using N2_CO2 = BinaryCoeff::N2_CO2;
	// using H2O_N2 = BinaryCoeff::H2O_N2<Scalar, CO2Table, true>;
   // using GridView = GetPropType<TypeTag, Properties::GridView>;
   // enum {
   //	dim = GridView::dimension,
   //     dimWorld = GridView::dimensionworld,
   // };
   // using GlobalPosition = Dune::FieldVector<Scalar, dimWorld>; //added by du
...
//    static const int numComponents = 9; // Brine, TC, Na, Cl, Ca, Urea, Urease, TNH
    ...
    static const int numComponents = 8+8; // Water, N2,  Na, h, Ca, Fe2+, O2, Mg, Al, SiO2, HPO4, K, Mn, Ti(OH)4, cl, hco3,CO2/Ctot
    static const int numMajorComponents = 2; //Water, N2
    // static const int numSecComponents = 5; // Hco3, co3, CO2, h, oh
	static const int numSecComponents = 7; // 5; // 6; // h, co3, oh,  hco3, fe2, fe3 ,co2(aq)// Fe3+, // co2(g)
    ...
    // static const int CtotIdx  = 2;
    static const int CO2aqIdx  = 2;	
	// static const int HIdx  = 2;
    static const int O2Idx =  3;
	
    static const int ClIdx  =  4;
    static const int CaIdx  =  5;
    // static const int TNHIdx  = 6;
    static const int Fe2Idx =  6;
	static const int NaIdx  =  7;
    static const int MgIdx  = 8;
    static const int AlIdx  =  9;
    static const int MnIdx  =  10;
    static const int SiO2Idx  =  11;
    static const int HPO4Idx =  12;
	static const int TiOH4Idx =  13;
	static const int KIdx =  14;
    static const int HIdx =  15;
	// static const int ClIdx =  15;	

    // static const int NH4Idx = numComponents;
    // static const int Fe3Idx =  numComponents;
    static const int OHIdx =  numComponents;
    static const int HCO3Idx = numComponents + 1;
    static const int CO3Idx = numComponents + 2;
    static const int CO2aqonlyIdx =  numComponents + 3;
    static const int HonlyIdx =  numComponents + 4;	
    static const int Fe2onlyIdx =  numComponents + 5;
    static const int Fe3Idx =  numComponents + 6;	
    /*!
    ...
    static std::string componentName(int compIdx)
    {
		...
		// case CtotIdx: return "TotalC";
		// case TCaqIdx: return "TotalCaq";
        case CaIdx: return Ca::name();
        case NaIdx: return Na::name();
        case ClIdx: return Cl::name();
        // case TNHIdx: return "TotalNH";//NH3::name();
        case Fe2onlyIdx: return Fe2::name();
		case Fe3Idx: return Fe3::name();
	    case Fe2Idx: return  "TotalFe";	
        case MgIdx: return Mg::name();
        case AlIdx: return Al::name();
		case KIdx: return K::name();
		case MnIdx: return Mn::name();
        case HPO4Idx: return HPO4::name();
        case SiO2Idx: return SiO2::name();
        case TiOH4Idx: return TiOH4::name();
        // case NH4Idx: return NH4::name();
        case HIdx: return "TotalH";
        case HonlyIdx: return H::name();		
        case OHIdx: return OH::name();
        case HCO3Idx: return HCO3::name();
        case CO3Idx: return CO3::name();
		case CO2aqonlyIdx: return "CO2aq";
		case CO2aqIdx: return "TotalC";
        // case CO2gIdx: return "CO2g";
...
// only the molar mass of water is returned.
		...
		// case CtotIdx: return CO2::molarMass();
        case CaIdx: return Ca::molarMass();
        case NaIdx: return Na::molarMass();
        case ClIdx: return Cl::molarMass();
        case MgIdx: return Mg::molarMass();
        case AlIdx: return Al::molarMass();
		case KIdx: return K::molarMass();
		case MnIdx: return Mn::molarMass();
        case HPO4Idx: return HPO4::molarMass();
        case SiO2Idx: return SiO2::molarMass();
        case TiOH4Idx: return TiOH4::molarMass();
        // case UreaseIdx: return Urease::molarMass();
        // case UreaIdx: return Urea::molarMass();
        case Fe2Idx: return Fe2::molarMass();
		case Fe2onlyIdx: return Fe2::molarMass();
        case Fe3Idx: return Fe3::molarMass();
        // case TNHIdx: return NH3::molarMass();
        // case NH4Idx: return NH4::molarMass();

        case CO2aqIdx: return CO2::molarMass();
        case CO2aqonlyIdx: return CO2::molarMass();
		// // case CO2gIdx: return CO2::molarMass();
        case HIdx: return H::molarMass();
        case HonlyIdx: return H::molarMass();		
    ...
    static Scalar charge(int compIdx)
    {
		...
		case Fe2onlyIdx: return Fe2::charge();
        case Fe3Idx: return Fe3::charge();
        case MgIdx: return Mg::charge();
        case AlIdx: return Al::charge();
		case KIdx: return K::charge();
		case MnIdx: return Mn::charge();
        case HPO4Idx: return HPO4::charge();
        case SiO2Idx: return SiO2::charge();
        case TiOH4Idx: return TiOH4::charge();
        case HIdx: return H::charge();
        case HonlyIdx: return H::charge();		
        case OHIdx: return OH::charge();
        case CO2aqIdx: return 0;
        case CO2aqonlyIdx: return 0;
		// // case CO2gIdx: return 0;
...
public:
        ...
        else if (phaseIdx == nPhaseIdx)
        {
            ...
            // assume ideal mixture: steam, nitrogen and oxygen don't "see" each other
                ...
                return H2O::gasDensity(T, fluidState.partialPressure(nPhaseIdx, H2OIdx))
                       ...
                       + O2::gasDensity(T, fluidState.partialPressure(nPhaseIdx, O2Idx))
    ...
    static Scalar molarDensity(const FluidState &fluidState, int phaseIdx)
    {
        ...
        else
        {
            ...
            return H2O::gasMolarDensity(T, fluidState.partialPressure(nPhaseIdx, H2OIdx))
				   ...
				   + O2::gasMolarDensity(T, fluidState.partialPressure(nPhaseIdx, O2Idx))
                            ...
                            int phaseIdx)
    {
        ...
        if (Policy::useN2ViscosityAsGasMixtureViscosity())
        {
			...
			// printf("The value of N2::gasViscosity(T, p) is: %.e\n", N2::gasViscosity(T, p)); //2e-5
                                      ...
                                      int compIdx)
    {
		...
		// Scalar co2aq_ = fluidState.moleFraction(wPhaseIdx, CO2aqIdx)*55344.789;
		// Scalar co2aqonly_ = fluidState.moleFraction(wPhaseIdx, CO2aqonlyIdx)*55344.789;
		// double ratio = co2aqonly_ / co2aq_;
		// if (co2aq_ == 0 || co2aqonly_ ==0)
		// {
		// 	ratio = 2.38640000e-07 / 2.65687305e-06;
		// 	// printf("ratio1: %.5e\n",ratio);
		// }
		// printf("ratio1: %.5e\n",ratio);
		// Scalar k1_ = 4.52168e-7;
        // Scalar k2_ = 4.69029e-11;
		// Scalar kw_ = 1e-14;
        // liquid phase
		// printf("The value of H2O_N2::henry(T)/p is: %.8e\n", BinaryCoeff::H2O_N2::henry(T)/p);
		// printf("The value of H2O_CO2::henry(T)/p is: %.8e\n", BinaryCoeff::H2O_CO2::henry(T)/p);
		// printf("The value of H2O::vaporPressure(T)/p is: %.8e\n", H2O::vaporPressure(T)/p);
		if (phaseIdx == wPhaseIdx)
        {
            switch(compIdx){
				...
				// case CtotIdx: return BinaryCoeff::H2O_CO2::henry(T)/p;
                // case CO2aqonlyIdx: return BinaryCoeff::H2O_CO2::henry(T)/p;
				case CO2aqIdx: return BinaryCoeff::H2O_CO2::henry(T)/p; 
                // case CO2aqIdx: return BinaryCoeff::H2O_CO2::henry(T)/p * ratio;
				
				// printf("The value of coefiicient is: %.8e\n", ((k1_*k2_) /(h_*h_) + k1_ / h_ + 1 ));
				// printf("The value of H2O_CO2::henry(T)/p is: %.8lf\n", BinaryCoeff::H2O_CO2::henry(T)/p);
				// 
				case O2Idx: return BinaryCoeff::H2O_O2::henry(T)/p;
				case CaIdx: return 0.0;
        		case NaIdx: return 0.0;
        		case ClIdx: return 0.0;
        		case Fe2Idx: return 0.0;
				case MgIdx: return 0.0;
        		case AlIdx: return 0.0;
        		case MnIdx: return 0.0;
        		case KIdx: return 0.0;
        		case SiO2Idx: return 0.0;
        		case HPO4Idx: return 0.0;
        		case TiOH4Idx: return 0.0;
        ...
        return 1.0;
        // printf("The value of H2O::vaporPressure(T)/p is: %.8lf\n", H2O::vaporPressure(T)/p);
        // printf("The value of H2O_N2::henry(T)/p is: %.8lf\n", BinaryCoeff::H2O_N2::henry(T)/p);
        // printf("The value of H2O_CO2::henry(T)/p is: %.8lf\n", BinaryCoeff::H2O_CO2::henry(T)/p);
   ...
   using Base::diffusionCoefficient;
...

    {
        ...
        if (phaseIdx == nPhaseIdx) {
			...
			// {
			// 	printf("The value of H2O_N2::gasDiffCoeff(T, p) is: %.e\n", BinaryCoeff::H2O_N2::gasDiffCoeff(T, p));
			// 	return BinaryCoeff::H2O_N2::gasDiffCoeff(T, p);
			// 	// return 0.5;
            // }
			// if (compIIdx == N2Idx && compJIdx == CO2aqonlyIdx)
            // {
			//     // printf("reached here");
			// 	// printf("The value of N2_CO2::liquidDiffCoeff(T, p) is: %.e\n", N2_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p));
			// 	return BinaryCoeff::N2_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p);
			// 	// return 0.0;
		    // }
			...
			if (compIIdx == N2Idx && compJIdx == CO2aqIdx)
            {
			    // printf("reached here");
				// printf("The value of N2_CO2::liquidDiffCoeff(T, p) is: %.e\n", N2_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p));
				return BinaryCoeff::N2_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p); // 2e-5
				// return 4e-7;//0.0;
		    }
            if (compIIdx == N2Idx && compJIdx == O2Idx)
			{
				// printf("The value of N2_O2::gasDiffCoeff(T, p) is: %.e\n", BinaryCoeff::N2_O2::gasDiffCoeff(T, p));
				return BinaryCoeff::N2_O2::gasDiffCoeff(T, p);
				// return 0.5;
            }
			// if (compIIdx == H2OIdx && compJIdx == CO2Idx)
            // {
			// 	printf("not reach here");
			// 	// printf("The value of H2O_CO2::liquidDiffCoeff(T, p) is: %.e\n", H2O_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p));
			// 	return H2O_CO2::gasDiffCoeff<Scalar, CO2Table>(T, p);
			// }
        ...
        // liquid phase
			...
			{
				// printf("reached here");
				// printf("The value of H2O_O2::liquidDiffCoeff(T, p) is: %.e\n", BinaryCoeff::H2O_O2::liquidDiffCoeff(T, p));
                                      ...
                                      const int phaseIdx)
    {
        ...
        else
        {
            ...
            if (!Policy::useN2HeatConductivityAsGasMixtureHeatConductivity())
            {
                ...
                return lambdaN2 + lambdaH2O + lambdaCO2 + lambdaO2;
            }
            else
                return lambdaPureN2;
        }
                               ...
                               int phaseIdx)
    {
        ...
        if (!Policy::useIdealGasHeatCapacities()) {
            ...
            Scalar c_vO2molar = Constants::R*2.51; // 
